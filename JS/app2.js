...
const ml_section = document.querySelector(".mailestones");
...
prev_btn.addEventListener("click", () => {
    if (currentIndex === 0) {
        currentIndex = 9; //need change the number once the photo added
    } else {
        currentIndex--;
    }
    changeImage(currentIndex);
...
next_btn.addEventListener("click", () => {
    if (currentIndex === 9) {
        currentIndex = 0; //need change the number once the photo added
    } else {
        currentIndex++;
    }
    changeImage(currentIndex);
...
function activeLink() {
    ...
    let passedSections = Array.from(sections).map((sct, i) => {
        return {
            y: sct.getBoundingClientRect().top - header.offsetHeight, id: i,
        };
    }).filter((sct) => sct.y <= 0);
